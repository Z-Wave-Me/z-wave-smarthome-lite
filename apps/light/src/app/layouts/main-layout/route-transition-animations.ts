...
export const routeTransitionAnimations = trigger('mainRouteAnimation', [
  ...
  transition('* => control', [
    ...
    group([
      ...
      query(':enter', [animate('.3s ease-out', style({ transform: 'translateY(0%)' })), animateChild()], {
        optional: true,
      }),
    ]),
  ]),
  transition('control => *', [
    ...
    group([
      query(
        ':leave',
        [style({ transform: 'translateY(0%)' }), animate('.3s ease-out', style({ transform: 'translateY(100%)' }))],
        { optional: true },
      ),
      query(':enter', [animate('.3s ease-out', style({ opacity: '1' })), animateChild()], { optional: true }),
    ]),
  ]),
]);
