...
import { exhaustMap, filter, first, map, mergeMap, switchMap, toArray } from 'rxjs/operators';
...
export class RoomSubMenuComponent implements OnInit {
  ...
  constructor(private readonly store: Store, private readonly activatedRoute: ActivatedRoute) {
    this.info$ = activatedRoute.url.pipe(
      ...
      switchMap((location) => store.select((state) => state.locations.entities[location])),
      filter((data) => !!data),
      map(({ imgSrc, title, main_sensors: mainSensors }) => ({ imgSrc, title, mainSensors })),
    );
  }
